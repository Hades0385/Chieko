...
function initRouteFromURL() {
  ...
  const savedId = localStorage.getItem('selectedRouteId');

  // 優先使用網址參數，其次使用儲存紀錄
  let route = null;
  if (idParam) {
    route = ROUTES.find(r => r.id.toString() === idParam);
  } else if (savedId) {
    route = ROUTES.find(r => r.id.toString() === savedId);
  }
  ...
  // 更新頁面顯示名稱
  const titleEl = document.getElementById('route_name');
  if (titleEl) titleEl.textContent = route.name || route.label || '未命名路線';
